...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reserve_car'])) {
        // Store session data only when reserve_car form is submitted
		...
		<?php
			if(isset($_POST['submit_reservation']) && $_SERVER['REQUEST_METHOD'] === 'POST')
			{
                ...
                try
                {
					...
					$stmtgetCurrentClientID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'GcampDB' AND TABLE_NAME = 'clients'");
